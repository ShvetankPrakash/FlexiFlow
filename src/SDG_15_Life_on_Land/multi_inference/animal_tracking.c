...
char Read_Input_Signal_Respond() {
    ...
    char check_output[Message_Bits + 1];
    demodulate_fsk_signal(output_signal, check_output);

    if (string_compare(check_output, USER_ID)) {
        // We should see consistency if we demodulate the modulated response
        // This works as checking for functional correctness
